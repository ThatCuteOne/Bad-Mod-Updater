...
class ModEntry():
    ...
    async def get_metadata_from_jar(self,file):
            try:
                    ...
                    if "fabric.mod.json" in jar.namelist():
                        with jar.open("fabric.mod.json") as f:
                            ...
                            return {
                                'id': data["id"],
                                'name': data["name"],
                                'sha512': await self.calculate_hash(file)
                            }
    ...
    @staticmethod
    async def calculate_hash(file:Path):
...
class ModIndex():
    ...
    async def remove_by_hash(self,target_hash):
        new_index = []
    ...
    async def add_version(self,version):
        ...
        await self.save()
    @staticmethod
    async def search_in_index(index,file):
        target_hash = await ModEntry.calculate_hash(file)
        ...
        return None
    async def key_in_versions(self,key:str,target_key):
...
class ModManager():
    def __init__(self):
        ...
        self.user = User()
    async def get_mod(self,version:dict):
        new_version = await self.api.get_newest_version(version)
        if not new_version:
            ...
            return False
        await self.resolve_dependencies(new_version)
        for file in new_version['files']:
            if file["hashes"]['sha512'] == version['mod_file']["hashes"]['sha512'] and file["primary"] == True:
                ...
                await self.install_mod(new_version,version)
                return True
    ...
    async def resolve_dependencies(self,parent_version:dict):
        ...
        for dependency in dependencies:
            d = await self.index.key_in_versions('project_id',dependency['project_id'])
            if d:
                try:
                ...
                version = await self.api.get_newest_version(dependency)
                if not version:
                    print(f"Couln't find compatible version for selected Minecraft Version: '{settings.MINECRAFT_VERSION}' and Loader: '{settings.MOD_LOADER}'")
                    return False
                version['metadata'] = await  self.api.get_project_info(version['project_id'])
                ...
                await self.install_new_mod(version)
                await self.resolve_dependencies(version)

    async def install_new_mod(self,version:dict):
        version = ModEntry._add_mod_file(version)
        await self.api.download_file(version['mod_file']['url'],settings.MODS_DIRECTORY / version['mod_file']['filename'])
        await self.index.add_version(version)
            
    async def install_mod(self,new_version,old_version):
        await self.api.download_file(new_version['mod_file']['url'],settings.MODS_DIRECTORY / new_version['mod_file']['filename'])
        await self.index.remove_by_hash(old_version['mod_file']['hashes']['sha512'])
        await aio_os.remove(settings.MODS_DIRECTORY / old_version['mod_file']['filename'])
        await self.index.add_version(new_version)

    async def get_mod_from_file(self,file):
        metadata = await ModEntry.get_metadata_from_jar(ModEntry(),file)
        project = await self.api.search_mod(metadata['name'])
        if not project:
            return False
        versions = await self.api.get_project_versions(project['project_id'])
        version = await self.index.search_in_index(versions,file)
        
        if not version:
            pass
            # new_version = await self.api.get_newest(versions)
            ...
            #                 await self.install_mod(new_version,{'mod_file':{'hashes':{'sha512':metadata['sha512']},'filename':file}})
            #                 return True
        else:
            ...
            await self.index.add_version(version)
            return await self.get_mod(version)



    async def update_mod(self,modfile):
            data = await self.index.search_in_index(self.index.versions, modfile)
            if data:
                print(f"Mod:'{data['metadata']['title']}'")
                return await self.get_mod(data)
            else:
                await self.get_mod_from_file(modfile)
