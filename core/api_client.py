...
class ModrinthAPI:
    ...
    def search_mod(
        modname: str,
        loader: str = settings.MOD_LOADER,
        minecraft_version: str = settings.MINECRAFT_VERSION
    ) -> Optional[Dict]:
        ...
        if not results:
            ...
            return results[0]
    @staticmethod
    def get_newest_version(project_id:str):
        """Returns the newest mod version(if compatible)
        Args:
            version(ModVersion) : local mod version
        """
        ...
        if not compatible_versions:
            ...
            return a[0]
    @staticmethod
    def get_project_versions(project_id: str) -> List[ModVersion]:
        url = f"https://api.modrinth.com/v2/project/{project_id}/version"
        response = requests.get(url)
        response.raise_for_status()
